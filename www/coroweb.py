...
# 这个字典里的key是即为参数名，str类型;value是一个inspect.Parameter类型的对象，包含的一个参数的各种信息
# inspect.Parameter.kind是一个_ParameterKind枚举类型的对象，值为这个参数的类型（可变参数VAR_KEYWORD通过**前缀来声明，关键词参数KEYWORD_ONLY，etc）
# 参数类型：http://www.imooc.com/article/13053
# inspect.Parameter.default：如果这个参数有默认值，即返回这个默认值，如果没有，返回一个inspect._empty类
...
def add_routes(app, module_name):
    ...
    # 如果handlers模块在handler目录下那么传入的module_name就是handler.handlers
